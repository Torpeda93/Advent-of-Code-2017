...
vector<set<int>> setRows;

const int table[TAB_SIZE][TAB_SIZE] =
{ 
...
void fillSets()
{
    ...
    while (i != TAB_SIZE)
    {
        auto tempRow = set<int>(table[i], table[i] + TAB_SIZE);
        setRows.push_back(tempRow);
        ++i;
    }
}
...
int maxMinValDiff()
{
    ...
    while (i != TAB_SIZE)
    {
        auto setIt = *setRows[i].begin();
        auto setIt2 = *setRows[i].rbegin();
        sum += setIt2 - setIt;
        ++i;
    }
...
int divFinder()
{
    int modVal = 0;
    int divVal = 0;
    for (auto && tmpRow : setRows)
    {
        for (auto && firstVal : tmpRow)
        {
            for (auto && secondVal : tmpRow)
            {
                if (firstVal > secondVal)
                {
                    modVal = firstVal % secondVal;
                    if (0 == modVal)
                    {
                        divVal += firstVal / secondVal;
                    }
                }
            }
        }
...
int main()
{
    fillSets();
    { //Part 1
        cout << "Solution 1: " << maxMinValDiff() << endl;
    }
    { //Part 2
        cout << "Solution 2: " << divFinder() << endl;
    }
    return 0;
}